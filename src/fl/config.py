...
class ConfigManager:
    ...
    def generate_exp_key(config):
        ...
        key = f"[{model_name}][rounds{num_rounds}][epochs-{config['sft_config_args']['num_train_epochs']}][clients{num_clients}-per-round-{per_round}][{config['dataset']['labels_to_keep']}-{config['dataset']['classes_per_client']}]"
        if config['use_lora']:
