...
def plot_gemma_all_datasets_subplots():
    ...
    for idx, (domain_key, domain_label) in enumerate(DOMAIN_NAMES.items()):
        ...
        if domain_key in datasets_data_with_grad:
            ...
            print(f"{domain_label}- Overall Mean Accuracy with Gradients: {np.mean(grad_accs):.2f}%\n")
        ...
        if domain_key in datasets_data_no_grad:
            ...
            print(f"{domain_label}- Overall Mean Accuracy without Gradients: {np.mean(no_grad_accs):.2f}%\n")
            print(f"{'--' * 60}\n")
    ...
    print(f"Average Overall Mean Accuracy with Gradients across all domains: {avg_grads_acc:.2f}%")
    print(f"Average Overall Mean Accuracy without Gradients across all domains: {avg_no_grads_acc:.2f}%")
